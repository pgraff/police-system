...
/**
 * NATS query handler for officer projection.
 * Handles synchronous query requests from the edge service.
 */
@Component
public class OfficerNatsQueryHandler {

    private static final Logger log = LoggerFactory.getLogger(OfficerNatsQueryHandler.class);
    ...
    public OfficerNatsQueryHandler(NatsProperties properties,
    ...
    public void start() {
        ...
        try {
            ...
            String prefix = properties.getQuerySubjectPrefix().isBlank() 
                    ? "query.officer" 
                    : properties.getQuerySubjectPrefix() + ".officer";
            querySubject = prefix + ".>";
    ...
    private void handleExistsQuery(Message message, String payload) {
        try {
            ...
            String badgeNumber = request.getResourceId();
            
            boolean exists = projectionService.getProjection(badgeNumber).isPresent();
            ...
            if (message.getReplyTo() != null) {
                ...
                log.debug("Responded to exists query for badgeNumber {}: exists={}", badgeNumber, exists);
            } else {
    ...
    private void handleGetQuery(Message message, String payload) {
        try {
            ...
            Object data = projectionService.getProjection(badgeNumber)
            ...
            if (message.getReplyTo() != null) {
                connection.publish(message.getReplyTo(), responseJson.getBytes(StandardCharsets.UTF_8));
                log.debug("Responded to get query for badgeNumber {}: found={}", badgeNumber, data != null);
            } else {
    ...
    private void sendErrorResponse(Message message, String errorMessage) {
        try {
            // Try to extract queryId from request if possible
