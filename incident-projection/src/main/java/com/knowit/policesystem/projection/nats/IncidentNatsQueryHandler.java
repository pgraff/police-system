...
/**
 * NATS query handler for incident projection.
 * Handles synchronous query requests from the edge service.
 */
@Component
public class IncidentNatsQueryHandler {

    private static final Logger log = LoggerFactory.getLogger(IncidentNatsQueryHandler.class);
    ...
    public IncidentNatsQueryHandler(NatsProperties properties,
    ...
    public void start() {
        ...
        try {
            ...
            String prefix = properties.getQuerySubjectPrefix().isBlank() 
                    ? "query.incident" 
                    : properties.getQuerySubjectPrefix() + ".incident";
            querySubject = prefix + ".>";
    ...
    private void handleExistsQuery(Message message, String payload) {
        try {
            ...
            String incidentId = request.getResourceId();
            
            boolean exists = projectionService.getProjection(incidentId).isPresent();
            ...
            if (message.getReplyTo() != null) {
                ...
                log.debug("Responded to exists query for incidentId {}: exists={}", incidentId, exists);
            } else {
    ...
    private void handleGetQuery(Message message, String payload) {
        try {
            ...
            Object data = projectionService.getProjection(incidentId)
            ...
            if (message.getReplyTo() != null) {
                connection.publish(message.getReplyTo(), responseJson.getBytes(StandardCharsets.UTF_8));
                log.debug("Responded to get query for incidentId {}: found={}", incidentId, data != null);
            } else {
