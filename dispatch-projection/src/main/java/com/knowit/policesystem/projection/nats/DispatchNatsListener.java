...
import com.knowit.policesystem.projection.consumer.DispatchEventParser;
...
@Component
public class DispatchNatsListener {

    private static final Logger log = LoggerFactory.getLogger(DispatchNatsListener.class);
    ...
    public DispatchNatsListener(NatsProperties properties,
        ...
        try {
            ...
            dispatcher = connection.createDispatcher(message -> {
                try {
                    ...
                    log.error("Failed to process NATS dispatch event", e);
                }
            });
            subject = properties.getSubjectPrefix().isBlank()
                    ? "commands.dispatch.>"
                    : properties.getSubjectPrefix() + ".dispatch.>";
