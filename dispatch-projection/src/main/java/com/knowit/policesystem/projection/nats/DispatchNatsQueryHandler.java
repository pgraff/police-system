...
/**
 * NATS query handler for dispatch projection.
 * Handles synchronous query requests from the edge service.
 */
@Component
public class DispatchNatsQueryHandler {

    private static final Logger log = LoggerFactory.getLogger(DispatchNatsQueryHandler.class);
    ...
    public DispatchNatsQueryHandler(NatsProperties properties,
                                    DispatchProjectionService projectionService,
    ...
    public void start() {
        ...
        try {
            ...
            String prefix = properties.getQuerySubjectPrefix().isBlank() 
                    ? "query.dispatch" 
                    : properties.getQuerySubjectPrefix() + ".dispatch";
            querySubject = prefix + ".>";
    ...
    private void handleExistsQuery(Message message, String payload) {
        try {
            ...
            boolean exists = projectionService.getProjection(dispatchId).isPresent();
            ...
            if (message.getReplyTo() != null) {
                ...
                log.debug("Responded to exists query for dispatchId {}: exists={}", dispatchId, exists);
            } else {
