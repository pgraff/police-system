...
public class ActivityProjectionRepository {
    ...
    public Optional<ActivityProjectionEntity> findByActivityId(String activityId) {
        List<ActivityProjectionEntity> result = jdbcTemplate.query("""
                SELECT activity_id, activity_time, activity_type, description, status, completed_time, updated_at
    ...
    private void upsertMerged(ActivityProjectionEntity entity) {
        jdbcTemplate.update("""
                        INSERT INTO activity_projection (activity_id, activity_time, activity_type, description, status, completed_time, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                        ON CONFLICT (activity_id) DO UPDATE SET
