...
/**
 * NATS query handler for activity projection.
 * Handles synchronous query requests from the edge service.
 */
@Component
public class ActivityNatsQueryHandler {

    private static final Logger log = LoggerFactory.getLogger(ActivityNatsQueryHandler.class);
    ...
    public ActivityNatsQueryHandler(NatsProperties properties,
    ...
    public void start() {
        ...
        try {
            ...
            String prefix = properties.getQuerySubjectPrefix().isBlank() 
                    ? "query.activity" 
                    : properties.getQuerySubjectPrefix() + ".activity";
            querySubject = prefix + ".>";
    ...
    private void handleExistsQuery(Message message, String payload) {
        try {
            ...
            String activityId = request.getResourceId();
            
            boolean exists = projectionService.getProjection(activityId).isPresent();
            ...
            if (message.getReplyTo() != null) {
                ...
                log.debug("Responded to exists query for activityId {}: exists={}", activityId, exists);
            } else {
    ...
    private void handleGetQuery(Message message, String payload) {
        try {
            ...
            Object data = projectionService.getProjection(activityId)
            ...
            if (message.getReplyTo() != null) {
                connection.publish(message.getReplyTo(), responseJson.getBytes(StandardCharsets.UTF_8));
                log.debug("Responded to get query for activityId {}: found={}", activityId, data != null);
            } else {
