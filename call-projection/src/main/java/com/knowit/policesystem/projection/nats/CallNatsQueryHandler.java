...
/**
 * NATS query handler for call projection.
 * Handles synchronous query requests from the edge service.
 */
@Component
public class CallNatsQueryHandler {

    private static final Logger log = LoggerFactory.getLogger(CallNatsQueryHandler.class);
    ...
    public CallNatsQueryHandler(NatsProperties properties,
    ...
    public void start() {
        ...
        try {
            ...
            String prefix = properties.getQuerySubjectPrefix().isBlank() 
                    ? "query.call" 
                    : properties.getQuerySubjectPrefix() + ".call";
            querySubject = prefix + ".>";
    ...
    private void handleExistsQuery(Message message, String payload) {
        try {
            ...
            String callId = request.getResourceId();
            
            boolean exists = projectionService.getProjection(callId).isPresent();
            ...
            if (message.getReplyTo() != null) {
                ...
                log.debug("Responded to exists query for callId {}: exists={}", callId, exists);
            } else {
    ...
    private void handleGetQuery(Message message, String payload) {
        try {
            ...
            Object data = projectionService.getProjection(callId)
            ...
            if (message.getReplyTo() != null) {
                connection.publish(message.getReplyTo(), responseJson.getBytes(StandardCharsets.UTF_8));
                log.debug("Responded to get query for callId {}: found={}", callId, data != null);
            } else {
