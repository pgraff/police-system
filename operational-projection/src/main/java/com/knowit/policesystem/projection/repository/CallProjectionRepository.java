...
public class CallProjectionRepository {
    ...
    public void linkToIncident(LinkCallToIncidentRequested event, Instant updatedAt) {
        jdbcTemplate.update("""
                        UPDATE call_projection
    ...
    public void linkToDispatch(LinkCallToDispatchRequested event, Instant updatedAt) {
        // Note: LinkCallToDispatchRequested links a call to a dispatch.
        // The link is maintained in dispatch_projection.call_id, not in call_projection.
        // This method is a no-op because the dispatch repository will handle updating
        // the dispatch's call_id field. The projection service should route
        // LinkCallToDispatchRequested events to DispatchProjectionRepository instead.
    }

    public Optional<CallProjectionEntity> findByCallId(String callId) {
        List<CallProjectionEntity> result = jdbcTemplate.query("""
                SELECT call_id, call_number, priority, status, received_time, dispatched_time, arrived_time, cleared_time, description, call_type, incident_id, updated_at
    ...
    public List<CallProjectionEntity> findByIncidentId(String incidentId) {
    ...
    private void upsertMerged(CallProjectionEntity entity) {
        jdbcTemplate.update("""
                        INSERT INTO call_projection (call_id, call_number, priority, status, received_time, dispatched_time, arrived_time, cleared_time, description, call_type, incident_id, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ON CONFLICT (call_id) DO UPDATE SET
                            ...
                            incident_id = COALESCE(EXCLUDED.incident_id, call_projection.incident_id),
                            updated_at = EXCLUDED.updated_at
                        """,
                entity.callId(),
                entity.callNumber(),
