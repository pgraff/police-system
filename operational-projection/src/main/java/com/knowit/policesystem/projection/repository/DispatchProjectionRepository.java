...
public class DispatchProjectionRepository {
    ...
    public void linkToCall(LinkCallToDispatchRequested event, Instant updatedAt) {
        jdbcTemplate.update("""
                        UPDATE dispatch_projection
                        SET call_id = ?, updated_at = ?
                        WHERE dispatch_id = ?
                        """,
                event.getCallId(),
                ts(updatedAt),
    ...
    public Optional<DispatchProjectionEntity> findByDispatchId(String dispatchId) {
        List<DispatchProjectionEntity> result = jdbcTemplate.query("""
                SELECT dispatch_id, dispatch_time, dispatch_type, status, call_id, updated_at
    ...
    public List<DispatchProjectionEntity> findByCallId(String callId) {
    ...
    private void upsertMerged(DispatchProjectionEntity entity) {
        jdbcTemplate.update("""
                        ...
                        ON CONFLICT (dispatch_id) DO UPDATE SET
                            ...
                            call_id = COALESCE(EXCLUDED.call_id, dispatch_projection.call_id),
                            updated_at = EXCLUDED.updated_at
                        """,
                entity.dispatchId(),
